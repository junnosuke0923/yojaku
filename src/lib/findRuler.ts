 ...
 * ざらつきにも欠けにも強い。
 *
 * すぼまりが 1.03（傾き3度ほど）を超えたら `tilted` を立て、
 * 長方形（`rect`）に加えて**台形**（`quad`）も返す。
 * 台形で「ゆがみに合わせる」と効き目は大きい——
 * 傾き20度で、型紙の幅のずれが **+11.7% から +2.5% へ**下がった。
 * 逆に真上から撮れているときは、長方形＋相似のほうが正確である（+0.1% 対 +1.8%）。
 *
 * **ただし `tilted` が立っても、アプリは自動では切り替えない。**
 * 実写の見本では、長方形で渡せば型紙3枚とも 0.2 度以内でまっすぐ立つ
 * ＝ほとんど傾いていないのに、すぼまりは 1.0686 と出た。
 * 1.03 という境目は合成画像で決めた値で、実写の測り誤差に届いていない。
 * この節の頭に書いたとおり、ここが出すのは**提案であって、決定ではない**。
 *
 * ## 確かめた範囲
 *
