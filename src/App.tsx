...
export function App() {
  ...
  */
  ...
  /**
   * 画面が変わったら、いちばん上から見せる（依頼者の指示・2026-09-02）。
   *
   *   「このパーツを取り込むをした後に縫い代セクションに移動しますが、
   *     表示がそのセクションの一番上ではなく、中途半端な位置での表示となりました」
   *
   * 画面を入れかえても、巻き物の位置はそのまま残る。
   * 取り込むボタンは長い確認の画面のいちばん下にあるので、
   * 押したあとの縫い代の画面は、下のほうから始まってしまっていた。
   *
   * 見ているのは段階だけではない。**「測る」は1つの段階で2つの画面を持つ**ので、
   * 四隅を合わせる／確かめて取り込む の入れかわりも数に入れる。
   * ここを段階だけで見ていると、行列の2枚目へ移ったとき
   * （段階は `ruler` のまま）に効かない
   */
  const screen = step === 'ruler' ? (shown ? 'ruler-result' : 'ruler-quad') : step
  useEffect(() => {
    window.scrollTo(0, 0)
  }, [screen])
