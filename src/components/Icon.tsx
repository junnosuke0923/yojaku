...
const SHAPES: Record<IconName, ReactNode> = {
  ...
  */
  ...
  /**
   * 一方裁ちの生地。上下の向きが決まっている。
   *
   * 矢は**下向き**（依頼者の指示・2026-09-05「一方裁ちに設定した場合の
   * アイコンに書かれている矢印も下向きにして差し替えておいてください」）。
   * 型紙の地の目線も、一方裁ちのときは下向き一本で描いている
   * （`PatternMarks`）。同じことを表す絵は、同じ向きにそろえる
   */
  nap: (
    <>
      <rect x="4.4" y="2.9" width="15.2" height="18.2" rx="1.3" />
      <path d="M12 6.6v9.2M8.8 12.6L12 15.8l3.2-3.2" />
    </>
  ),
  ...
  */
  ...
  /*
    ...
    **この2つだけは自前で描かず、Lucide のものをそのまま使っている**
    （依頼者の指摘・2026-09-05「まだ矢印の描写がおかしいです。
      これは自前で描写しないといけないのですか？」）。

    自前では二度作り直して二度とも直せなかった。原因は形そのものにある——
    ほぼ一周する弧の終わりに矢じりを付けると、**羽根の片方が
    弧の上へ戻って重なり**、線が溶け合って鉤（かぎ）や小さなチェックに見える。
    接線に沿わせても、羽根を短くしても、細くしても消えなかった。
    Lucide のものは矢じりを弧の**外**へ出した直角の括弧にしてあり、
    どの大きさでも弧と混ざらない。

    出どころ: Lucide（rotate-ccw / rotate-cw）https://lucide.dev
    ライセンス: ISC。著作権表示を残せば、そのまま使ってよい。
    ...
    取り込み方は「線の形を2本、この表に書き写しただけ」。
    外から読み込むものは無いので、ネットにつながっていなくても出る
  */
  /*
    この2つだけは裁縫室の物ではなく、**端末の絵**（依頼者の指示・2026-09-13、
    ホーム画面に置くことを勧める札）。
    端末の話をしているので、端末の側で使われている絵に合わせる——
  ...
  */
  ...
  home: (
    <>
      <path d="M3.6 10.2L12 3.6l8.4 6.6v9.2a1.6 1.6 0 0 1-1.6 1.6H5.2a1.6 1.6 0 0 1-1.6-1.6z" />
      <path d="M9.4 21v-6.2h5.2V21" />
    </>
  ),

  /** iPhone の「共有」。箱から矢が上へ出ている形 */
  share: (
    <>
      <path d="M12 15.2V3.4" />
      <path d="M8.2 7.2L12 3.4l3.8 3.8" />
      <path d="M7.6 10.2H5.6A1.6 1.6 0 0 0 4 11.8v7.6A1.6 1.6 0 0 0 5.6 21h12.8a1.6 1.6 0 0 0 1.6-1.6v-7.6a1.6 1.6 0 0 0-1.6-1.6h-2" />
    </>
  ),
